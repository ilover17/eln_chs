...
export default class ScreenDetails extends Component {
  ...
  render() {
    ...
    const submitLabel = screen.isNew ? '创建' : '保存';

    const tabContentsMap = {
      properties: (
        <Tab eventKey="properties" title="属性" key={`properties_${screen.id}`}>
          {this.propertiesFields(screen)}
        </Tab>
      ),
      analyses: (
        <Tab eventKey="analyses" title="分析" key={`analyses_${screen.id}`}>
      ...
      research_plans: (
        <Tab eventKey="researchPlans" title="研究计划" key={`research_plans_${screen.id}`}>
          <ScreenResearchPlans
    ...
    return (
      ...
      >
        ...
        <Panel.Body>
          ...
          <ButtonToolbar>
            <Button bsStyle="primary" onClick={() => DetailActions.close(screen)}>关闭</Button>
