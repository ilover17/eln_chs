...
class MeasurementCandidate extends Component {
  ...
  _status() {
    if (this.props.id) {
      return (<span className='success'>成功：为样本创建了测量</span>);
    }
    if (this.props.errors.length > 0) {
      return (<span className='danger'>错误: {this.props.errors.join('. ')}</span>);
    }
  }
...
class ResearchPlanDetailsFieldTableMeasurementExportModal extends Component {
  ...
  _selectAllButton() {
    ...
    if (prefixes.length == 1) {
      ...
      return (
        <ButtonGroup>
          <Button onClick={() => this._selectAll.bind(this)()}>选择全部</Button>
