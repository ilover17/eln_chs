...
export default class WellplateDetails extends Component {
  ...
  render() {
    ...
    const submitLabel = wellplate.isNew ? '创建' : '保存';
    ...
    const tabContentsMap = {
      ...
      properties: (
        <Tab eventKey="properties" title="属性" key={`properties_${wellplate.id}`}>
          <WellplateProperties
            {...properties}
      ...
      analyses: (
        <Tab eventKey="analyses" title="分析" key={`analyses_${wellplate.id}`}>
