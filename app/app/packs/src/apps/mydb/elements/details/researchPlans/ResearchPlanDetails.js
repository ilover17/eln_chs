...
export default class ResearchPlanDetails extends Component {
  ...
  render() {
    ...
    const tabContentsMap = {
      research_plan: (
        <Tab eventKey="research_plan" title="研究计划" key={`rp_${researchPlan.id}`}>
      ...
      analyses: (
        <Tab eventKey="analyses" title="分析" key={`analyses_${researchPlan.id}`}>
          {this.renderAnalysesTab(researchPlan)}
        </Tab>
      ),
      attachments: (
        <Tab eventKey="attachments" title="附件" key={`attachments_${researchPlan.id}`}>
      ...
      wellplates: (
        <Tab eventKey="wellplates" title="布板" key={`wellplates_${researchPlan.id}`}>
      ...
      metadata: (
        <Tab eventKey={4} title="元数据" disabled={researchPlan.isNew} key={`metadata_${researchPlan.id}`}>
    ...
    return (
      <Panel bsStyle={researchPlan.isPendingToSave ? 'info' : 'primary'} className="eln-panel-detail research-plan-details">
        ...
        <Panel.Body>
          ...
          <ButtonToolbar>
            <Button bsStyle="primary" onClick={() => DetailActions.close(researchPlan)}>关闭</Button>
            {
              researchPlan.changed ? <Button bsStyle="warning" onClick={() => this.handleSubmit()}>{researchPlan.isNew ? '创建' : '保存'}</Button> : <div />
            }
          </ButtonToolbar>
        </Panel.Body>
      </Panel>
