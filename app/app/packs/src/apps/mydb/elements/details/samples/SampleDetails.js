...
export default class SampleDetails extends React.Component {
  ...
  initiateAnalysisButton(sample) {
    return (
      <div style={{ display: 'inline-block', marginLeft: '100px' }}>
        <DropdownButton id="InitiateAnalysis" bsStyle="info" bsSize="xsmall" title="启动分析">
  ...
  sampleHeader(sample) {
    ...
    return (
      <div>
        ...
        <OverlayTrigger
          placement="bottom"
          overlay={<Tooltip id="saveCloseSample">保存并关闭样本</Tooltip>}
        >
          <Button
            bsStyle="warning"
            bsSize="xsmall"
        ...
        <OverlayTrigger
          placement="bottom"
          overlay={<Tooltip id="saveSample">保存样本</Tooltip>}
        >
          <Button
            bsStyle="warning"
            bsSize="xsmall"
  ...
  samplePropertiesTab(ind) {
    ...
    return (
      <Tab eventKey={ind} title="属性" key={'Props' + sample.id.toString()}>
        <ListGroupItem>
          <SampleForm
            sample={sample}
            parent={this}
  ...
  sampleContainerTab(ind) {
    const { sample } = this.state;
    return (
      <Tab eventKey={ind} title="分析" key={'Container' + sample.id.toString()}>
  ...
  saveBtn(sample, closeView = false) {
    let submitLabel = (sample && sample.isNew) ? '创建' : '保存';
  ...
  sampleFooter() {
    ...
    const downloadAnalysesBtn = (sample.isNew || !hasAnalyses) ? null : (
      <Button bsStyle="info" disabled={!this.sampleIsValid()} onClick={() => this.handleExportAnalyses(sample)}>
        下载分析结果 {startExport ? <span>&nbsp;<i className="fa fa-spin fa-spinner" /></span> : null}
      </Button>
    );
