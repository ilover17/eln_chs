...
export default class ReactionDetails extends Component {
  ...
  reactionHeader(reaction) {
    ...
    const titleTooltip = `创建于： ${reaction.created_at} \n 更新于: ${reaction.updated_at}`;
    ...
    return (
      <div>
        ...
        <OverlayTrigger placement="bottom"
          overlay={<Tooltip id="saveReaction">保存并关闭反应</Tooltip>}>
          <Button
            bsStyle="warning"
            bsSize="xsmall"
        ...
        <OverlayTrigger placement="bottom"
          overlay={<Tooltip id="saveReaction">保存反应</Tooltip>}>
        ...
        <OverlayTrigger
          placement="bottom"
          overlay={<Tooltip id="generateReport">生成报告</Tooltip>}
        >
          <Button
            bsStyle="success"
            bsSize="xsmall"
  ...
  render() {
    ...
    const tabContentsMap = {
      scheme: (
        <Tab eventKey="scheme" title="计划" key={`scheme_${reaction.id}`}>
      ...
      properties: (
        <Tab eventKey="properties" title="属性" key={`properties_${reaction.id}`}>
      ...
      references: (
        <Tab eventKey="references" title="参照" key={`references_${reaction.id}`}>
      ...
      analyses: (
        <Tab eventKey="analyses" title="分析" key={`analyses_${reaction.id}`}>
    ...
    const submitLabel = (reaction && reaction.isNew) ? '创建' : '保存';
