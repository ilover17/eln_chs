        ...
        <Tabs activeKey={propTabKey} id="segment-prop-tabs" onSelect={this.propTabSelect}>
          <Tab eventKey={1} title="模板">
            <Panel show={showPropModal.toString()}>
              <Panel.Heading>
                <b>{`分段模板 [${element.label}]: ${element.desc}`}</b>&nbsp;
                ...
                <span className="button-right" >
                  <ButtonTooltip tip="上传JSON格式的分段模板" fnClick={this.handleUploadShow} element={element} place="top" fa="fa-upload" />&nbsp;
                  <ButtonTooltip txt="保存并发布" tip="保存并发布模板" fnClick={() => this.handleSubmit(true)} fa="fa-floppy-o" place="top" bs="primary" />&nbsp;
                  <ButtonTooltip txt="保存为草稿" tip="保存为草稿" fnClick={() => this.handleSubmit(false)} fa="fa-floppy-o" place="top" bs="primary" />
                </span>
      ...
      <tbody key={`tbody_${e.id}`}>
        <tr key={`row_${e.id}`} id={`row_${e.id}`} style={e.id === element.id ? { fontWeight: 'bold', borderWidth: 'medium', backgroundColor: 'white', borderStyle: 'groove' } : { fontWeight: 'unset' }}>
          ...
          <td>
            ...
            &nbsp;
            <ButtonTooltip tip="编辑分段属性" fnClick={this.editKlass} element={e} />
            &nbsp;
          </td>
          <td>{e.label}</td>
          <td>{e.desc}</td>
          ...
          <td>
            <ButtonTooltip tip="编辑分段模板" fnClick={this.handlePropShow} element={e} fa="fa-file-text" />&nbsp;
            <ButtonTooltip tip="以JSON格式编辑分段模板" fnClick={this.showJsonModal} element={e} fa="fa-file-code-o" />
          </td>
    ...
    return (
      <div>
        <Button bsStyle="primary" bsSize="small" onClick={() => this.newKlass()}>
          新的段落&nbsp;<i className="fa fa-plus" aria-hidden="true" />
        </Button>
        &nbsp;
        <br />
