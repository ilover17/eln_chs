    ...
    return (
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>数据收集器配置 - Device: {this.props.device.name}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Row style={rowStyle}>
        ...
        <Modal.Footer style={{ textAlign: 'left' }}>
          <Button bsStyle="primary" onClick={() => this.props.onClose()}>关闭</Button>
          <Button bsStyle="warning" onClick={() => this.handleSave(this.props.device)}>保存</Button>
        </Modal.Footer>
      </Modal>
    );
  }
