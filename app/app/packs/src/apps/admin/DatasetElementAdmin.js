...
export default class DatasetElementAdmin extends React.Component {
  ...
  newOption(key) {
    ...
    if (newOptionKey == null || newOptionKey.trim().length === 0) {
      notification({ title: '添加新选项', lvl: 'error', msg: 'please input option name first!' });
      return;
    }
    ...
    if (dupops && dupops.length > 0) {
      notification({ title: '添加新选项', lvl: 'error', msg: 'this option key is used already, please change another option key' });
      return;
    }
            ...
            <Panel show={showPropModal.toString()}>
              <Panel.Heading>
                ...
                <span className="button-right" >
                  ...
                  <ButtonTooltip txt="保存为草稿" tip="将模板另存为草稿" fnClick={() => this.handleSubmit(false)} fa="fa-floppy-o" place="top" bs="primary" />
                </span>
