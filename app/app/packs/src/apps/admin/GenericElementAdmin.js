            ...
            <div>
              <FormGroup bsSize="sm" style={{ marginBottom: 'unset', display: 'inline-table' }}>
                <InputGroup>
                  ...
                  />
                  <InputGroup.Button>
                    <OverlayTrigger placement="top" overlay={<Tooltip id={uuid.v4()}>添加新选项</Tooltip>}>
            ...
            <div>
              <FormGroup bsSize="sm" style={{ marginBottom: 'unset', display: 'inline-table' }}>
                <InputGroup>
                  ...
                  />
                  <InputGroup.Button>
                    <ButtonTooltip tip="添加新字段" fnClick={this.newField} element={{ layerKey }} fa="fa fa-plus" place="top" size="sm" />
                    <ButtonTooltip tip="添加虚拟字段" fnClick={this.onDummyAdd} element={{ l: layerKey, f: null }} fa="fa fa-plus-circle" place="top" size="sm" />
                  </InputGroup.Button>
                </InputGroup>
              </FormGroup>
            </div>
    ...
    if (showPropModal) {
      return (
        <Tabs activeKey={propTabKey} id="uncontrolled-tab-example" onSelect={this.propTabSelect}>
          <Tab eventKey={1} title="模板">
            <Panel show={showPropModal.toString()}>
              <Panel.Heading>
                <b>{`模板元素： [${element.name}]`}</b>&nbsp;
                ...
                <span className="button-right" >
                  <ButtonTooltip tip="上传JSON格式的元素模板" fnClick={this.handleUploadShow} element={element} place="top" fa="fa-upload" />&nbsp;
                  <ButtonTooltip txt="保存并发布" tip="保存并发布模板" fnClick={() => this.handleSubmit(true)} fa="fa-floppy-o" place="top" bs="primary" />&nbsp;
                  <ButtonTooltip txt="保存为草稿" tip="保存模板为草稿" fnClick={() => this.handleSubmit(false)} fa="fa-floppy-o" place="top" bs="primary" />
                </span>
    ...
    const tbody = els && els.map((e, idx) => (
      <tbody key={`tbody_${e.id}`}>
        <tr key={`row_${e.id}`} id={`row_${e.id}`} style={e.id === element.id ? { fontWeight: 'bold', borderWidth: 'medium', backgroundColor: 'white', borderStyle: 'groove' } : { fontWeight: 'unset' }}>
          <td>{idx + 1}</td>
          <td>
            <ButtonTooltip tip="拷贝到..." fa="fa fa-clone" element={e} fnClick={this.copyKlass} />
            &nbsp;
            <ButtonTooltip tip="编辑元素属性" element={e} fnClick={this.editKlass} />
            &nbsp;
          </td>
          <td>{e.name}</td>
          ...
          <td>{e.desc}</td>
          <td>
            <ButtonTooltip tip="编辑元素模板" fnClick={this.handlePropShow} element={e} fa="fa-file-text" />&nbsp;
            <ButtonTooltip tip="以JSON格式编辑元素模板" fnClick={this.showJsonModal} element={e} fa="fa-file-code-o" />
          </td>
