...
const buildCriteria = (props) => {
  ...
  const defaultName = (
    <Row>
      <Col md={6}>
        <GenProperties label="名称" value={genericEl.search_name || ''} type="text" onChange={event => props.onChange(event, 'search_name', '')} isEditable isRequired={false} />
      </Col>
      <Col md={6}>
        <GenProperties label="短标签" value={genericEl.search_short_label || ''} type="text" onChange={event => props.onChange(event, 'search_short_label', '')} isEditable isRequired={false} />
      </Col>
    </Row>
  );
