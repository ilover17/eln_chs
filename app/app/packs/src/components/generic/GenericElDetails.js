...
export default class GenericElDetails extends Component {
  ...
  propertiesTab(ind) {
    ...
    return (
      <Tab eventKey={ind} title="属性" key={`Props_${genericEl.id}`}>
  ...
  containersTab(ind) {
    const { genericEl } = this.state;
    return (
      <Tab eventKey={ind} title="分析" key={`Container_${genericEl.id}`}>
  ...
  attachmentsTab(ind) {
    const { genericEl } = this.state;
    return (
      <Tab eventKey={ind} title="附件" key={`Attachment_${genericEl.id}`}>
        <ListGroupItem style={{ paddingBottom: 20 }}>
          <GenericAttachments
  ...
  header(genericEl) {
    ...
    return (
      <div>
        ...
        <OverlayTrigger
          placement="bottom"
          overlay={<Tooltip id="saveScreen">保存</Tooltip>}
        >
          <Button
            bsStyle="warning"
            bsSize="xsmall"
  ...
  render() {
    const { genericEl } = this.state;
    const submitLabel = (genericEl && genericEl.isNew) ? '创建' : '保存';
