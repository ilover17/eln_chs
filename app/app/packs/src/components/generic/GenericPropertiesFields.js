  ...
  if (opt.value.is_new === true) {
    createOpt = (
      <div className="sample_radios">
        <OverlayTrigger placement="top" overlay={<Tooltip id={uuid.v4()}>与此样本关联</Tooltip>}>
          <Radio name={`dropS_${opt.value.el_id}`} disabled={opt.value.isAssoc === true} checked={opt.value.cr_opt === 0} onChange={() => opt.onChange({ ...opt.value, cr_opt: 0 })} inline>当前</Radio>
        </OverlayTrigger>
        <OverlayTrigger placement="top" overlay={<Tooltip id={uuid.v4()}>先从样本中分离，然后与之关联</Tooltip>}>
          <Radio name={`dropS_${opt.value.el_id}`} checked={opt.value.cr_opt === 1} onChange={() => opt.onChange({ ...opt.value, cr_opt: 1 })} inline>分离</Radio>
        </OverlayTrigger>
        <OverlayTrigger placement="top" overlay={<Tooltip id={uuid.v4()}>先复制样本，然后与之关联</Tooltip>}>
          <Radio name={`dropS_${opt.value.el_id}`} checked={opt.value.cr_opt === 2} onChange={() => opt.onChange({ ...opt.value, cr_opt: 2 })} inline>复制 </Radio>
        </OverlayTrigger>
      </div>
    );
  }
    ...
    <FormGroup>
      ...
      <FormControl.Static style={{ paddingBottom: '0px' }}>
        <div className={className}>
          ...
          {createOpt}
          <div>
            <OverlayTrigger placement="top" overlay={<Tooltip id={uuid.v4()}>移除</Tooltip>}>
